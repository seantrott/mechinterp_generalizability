---
title: "Emergence of Induction Heads Across Seeds"
author: "Sean Trott"
date: "August 15, 2025"
output:
  html_document:
...
directory_path <- "../../data/processed/induction_results/"
...
### Create head-level summaries
df_pythia_models_head_summaries = df_pythia_models %>%
  group_by(mpath, n_params, step, step_modded, seed, model,
           layer, head) %>%
  summarise(mean_ps = mean(ps),
            mean_ar_acc = mean(ar_acc),
            mean_ar_rank = mean(ar_mean_rank)) %>%
  mutate(seed_name = paste0("seed", seed))
...
## First token heads: a summary

```{r final_step_first_token}



df_by_head_seed = df_pythia_models_head_summaries %>%
  filter(step_modded == 143001) %>%
  group_by(mpath) %>%
  mutate(z_ps = scale(mean_ps)) 


### Max by layer
df_by_head_seed %>%
  group_by(mpath, seed, layer, n_params) %>%
  summarise(max_mean = max(mean_ps)) %>%
  ggplot(aes(x = layer,
             y = seed,
             fill = max_mean)) +
  geom_tile() +
  labs(x = "Layer",
       y = "Seed",
       fill = "Max Prefix-Matching Score") +
  scale_fill_gradient2(low = "blue",
                       mid = "white",
                       high = "red",
                       midpoint = 0, 
                       name = "Max Prefix-Matching Score") +
  theme_minimal() +
...
df_by_head_seed %>%
  ...
  labs(x = "Layer",
       y = "Seed",
       fill = "Max Prefix-Matching Score (z-scored)") +
  scale_fill_gradient2(low = "blue",
                       mid = "white",
                       high = "red",
                       midpoint = 0, 
                       name = "Max Prefix-Matching Score (z-scored)") +
  theme_minimal() +
  theme(text = element_text(size = 15),
        strip.text.y = element_text(angle = 0), # Keep facet labels horizontal
        axis.text.x = element_text(angle = 45, hjust = 1),
...
df_layerwise_attn <- df_by_head_seed %>%
  ...
  summarise(
    max_attention = max(z_ps),
    se_attention = sd(z_ps) / sqrt(n()),
    .groups = "drop"
  )
...
```{r layer_depth_first_token}
df_pythia_models_final_step = df_pythia_models_head_summaries %>%
  filter(step_modded == 143001) %>%
  group_by(model) %>%
...
summary_df <- df_pythia_models_final_step %>%
  ...
  summarise(
    max_ratio_per_seed = max(mean_ps, na.rm = TRUE),
    .groups = "drop"
  ) %>%
...
### Analyses
summary(lmer(mean_ps ~ prop_layer + (1|head) + (1|model), 
...
df_ar_by_step = df_pythia_models_head_summaries %>%
  group_by(model, n_params, step_modded, seed, seed_name) %>%
  summarise(mean_ar_acc = first(mean_ar_acc),
            mean_ar_rank = first(mean_ar_rank))


summary_avg_ar = df_ar_by_step %>%
  group_by(model, n_params, step_modded) %>%
  summarise(mean_across_seeds = mean(mean_ar_acc))
...
df_ar_by_step %>%
  ...
  geom_line(size = .6) +
  geom_line(data = summary_avg_ar, aes(x = step_modded, y = mean_across_seeds),
            color = "black", size = 1.5) +
  theme_minimal() +
  labs(x = "Training Step",
       y = "Associative Recall (Accuracy)",
       color = "") +
  scale_x_log10() +
...
## Track Max Prefix Score

```{r ps_over_time_ps}

### Track max PS at each time point for each seed, across layer/head
df_by_head_max_attention = df_pythia_models_head_summaries %>%
  group_by(model, n_params, step_modded, seed, seed_name) %>%
  slice_max(mean_ps)
...
summary_avg = df_by_head_max_attention %>%
  group_by(model, n_params, step_modded) %>%
  summarise(
    mean_across_seeds = mean(mean_ps)
  )
...
df_by_head_max_attention %>%
  ...
  theme_minimal() +
  labs(x = "Training Step",
       y = "Best Prefix-Matching Score (PS)",
       color = "") +
  scale_x_log10() +
  geom_vline(xintercept = 1000, 
           linetype = "dotted", 
...
summary(lmer(data = df_by_head_max_attention,
             mean_ps ~ log10(step_modded) + (1|seed_name) + (1|model),
             REML = FALSE))
...
```{r individual_heads_ps}
### 14M
df_pythia_models_head_summaries %>%
  filter(layer == 4) %>%
...
df_diff <- df_by_head_max_attention %>%
  ...
  mutate(
    ...
    d_ratio = c(NA, diff(mean_ps) / diff(log_step))
  ) %>%
  ungroup()
...
gam_all <- gam(mean_ps ~ s(log_step), data = df_by_head_max_attention)
...
grand_avg <- df_by_head_max_attention %>%
  group_by(step_modded) %>%
  summarise(grand_mean = mean(mean_ps))
...
df_by_head_max_attention %>%
  ...
  geom_line(data = grand_avg, aes(x = step_modded, y = grand_mean),
...
df_peak <- df_by_head_max_attention %>%
  ...
  summarise(max_attn = max(mean_ps))
...
slopes_df <- df_by_head_max_attention %>%
  ...
  nest() %>%
  mutate(
    fit = map(data, ~ lm(mean_ps ~ log_step, data = .x)),
...
```{r mds_induction}
df_by_layer = df_by_head_max_attention %>%
...
df_wide <- df_by_layer %>%
  ungroup() %>%
  mutate(model_id = paste0(model, "-", seed_name)) %>%
...
model_id_to_params <- df_by_head_max_attention %>%
    ungroup() %>%
    distinct(model, seed_name, n_params) %>%
    mutate(model_id = paste0(model, "-", seed_name)) %>%
...
cor_long <- cor_long %>%
  ...
  mutate(
    model1 = str_replace(Var1, "-seed\\d+$", ""),
    model2 = str_replace(Var2, "-seed\\d+$", ""),
...
cor_long = cor_long %>%
  left_join(df_by_head_max_attention %>% ungroup() %>% select(model, n_params) %>% distinct(),
            by = c("model1" = "model")) %>%
...
mds_by_step <- df_wide %>%
  ...
  unnest(mds_df) %>%
  separate(model_id, into = c("model", "seed_name"), sep = "-seed", remove = FALSE) %>%
  mutate(seed_name = paste0("seed", seed_name)) %>%
  rename(x = `...1`, y = `...2`) %>%
