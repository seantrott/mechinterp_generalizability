...
grand_avg_1back <- df_by_head_max_attention %>%
  group_by(step_modded) %>%
  summarise(grand_mean = mean(mean_1back))
...
df_by_head_max_attention %>%
  ...
  geom_line(data = summary_avg, 
            ...
            color = "black", size = 2) +
  geom_line(data = grand_avg_1back, 
...
## Compare previous and first-token attention variance

```{r}
### Track max previous token heads at each time point for each seed, across layer/head
df_by_head_max_attention_prev = df_pythia_models %>%
  group_by(model, n_params, step_modded, seed, seed_name) %>%
  slice_max(mean_1back)

### Track max previous token heads at each time point for each seed, across layer/head
df_by_head_max_attention_first = df_pythia_models %>%
  group_by(model, n_params, step_modded, seed, seed_name) %>%
  slice_max(mean_first)

df_by_head_max_attention_prev %>%
  group_by(model, n_params, step_modded) %>%
  summarise(
    sd_across_seeds = sd(mean_1back),
    mean_across_seeds = mean(mean_1back),
    cv = sd_across_seeds / mean_across_seeds
  ) %>%
  group_by(model, n_params) %>%
  summarise(
    mean_sd = mean(sd_across_seeds),
    mean_cv = mean(cv)
  )

df_by_head_max_attention_first %>%
  group_by(model, n_params, step_modded) %>%
  summarise(
    sd_across_seeds = sd(mean_first),
