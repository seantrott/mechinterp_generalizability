...
df_by_head_seed = df_pythia_models %>%
  ...
  group_by(mpath) %>%
  mutate(z_mean_first = scale(mean_first))


### Max by layer
df_by_head_seed %>%
  group_by(mpath, seed_name, Layer, n_params) %>%
  summarise(max_mean = max(mean_first)) %>%
  ggplot(aes(x = Layer,
             y = seed_name,
             fill = max_mean)) +
  geom_tile() +
  labs(x = "Layer",
       y = "Seed",
       fill = "Max first-token Attn.") +
  scale_fill_gradient2(low = "blue",
                       mid = "white",
                       high = "red",
                       midpoint = 0, 
                       name = "Max First-Token Attn.") +
  theme_minimal() +
...
df_by_head_seed %>%
  group_by(mpath, seed_name, Layer, n_params) %>%
  summarise(max_mean = max(z_mean_first)) %>%
  ggplot(aes(x = Layer,
             y = seed_name,
             fill = max_mean)) +
  geom_tile() +
  labs(x = "Layer",
       y = "Seed",
       fill = "Max First-Token Attn. (z-scored)") +
  scale_fill_gradient2(low = "blue",
                       mid = "white",
                       high = "red",
                       midpoint = 0, 
                       name = "Max First-Token Attn. (z-scored)") +
  theme_minimal() +
  theme(text = element_text(size = 15),
        strip.text.y = element_text(angle = 0), # Keep facet labels horizontal
        axis.text.x = element_text(angle = 45, hjust = 1),
...
df_layerwise_attn <- df_by_head_seed %>%
  ...
  summarise(
    max_attention = max(z_mean_first),
    se_attention = sd(z_mean_first) / sqrt(n()),
    .groups = "drop"
  )
...
summary_df <- df_pythia_models_final_step %>%
  ...
  summarise(
    max_ratio_per_seed = max(mean_first, na.rm = TRUE),
    .groups = "drop"
  ) %>%
...
### Analyses
summary(lmer(mean_first ~ prop_layer + (1|Head) + (1|model), 
...
summary(lmer(data = df_by_head_max_attention,
             mean_first ~ log10(step_modded) + (1|seed_name) + (1|model),
             REML = FALSE))
...
df_diff <- df_by_head_max_attention %>%
  ...
  mutate(
    ...
    d_ratio = c(NA, diff(mean_first) / diff(log_step))
  ) %>%
  ungroup()
...
gam_all <- gam(mean_first ~ s(log_step), data = df_by_head_max_attention)
...
grand_avg_first <- df_by_head_max_attention %>%
  group_by(step_modded) %>%
  summarise(grand_mean = mean(mean_first))

df_by_head_max_attention %>%
  ggplot(aes(x = step_modded,
             y = mean_first,
             color = factor(seed_name))) +
  geom_line(size = .6, alpha = .4) +
  geom_line(data = summary_avg, 
            aes(x = step_modded, y = mean_across_seeds), 
            color = "black", size = 2) +
  geom_line(data = grand_avg_first, 
            aes(x = step_modded, y = grand_mean), 
            inherit.aes = FALSE, 
            color = "red", size = 1.5) +
  theme_minimal() +
  labs(x = "Training Step (Log10)",
...
df_peak <- df_by_head_max_attention %>%
  ...
  summarise(max_attn = max(mean_first))
...
slopes_df <- df_by_head_max_attention %>%
  ...
  nest() %>%
  mutate(
    fit = map(data, ~ lm(mean_first ~ log_step, data = .x)),
